...
export default async function FieldPage() {
  ...
  const where = ["FIELD", "DRIVER"].includes(user.role)
  ...
  const todayRoutes = routes.filter(
    (route) => route.routeDate >= start && route.routeDate < end,
  );
      ...
      <div className="mt-8 space-y-6">
              ...
              <div className="divide-y divide-zinc-100">
                {route.stops.map((stop) => (
                  <Link
                    ...
                    className="grid gap-4 p-5 transition hover:bg-violet-50 sm:grid-cols-[48px_1fr_auto] sm:items-center"
                  >
                    <span
                      className={`grid size-11 place-items-center rounded-xl font-semibold ${
                        stop.status === "DONE"
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="font-semibold">{stop.store.name}</h3>
                        {stop.store.debt > 0 && (
        ...
        {visibleRoutes.length === 0 && (
          <div className="surface-card p-10 text-center text-zinc-500">
            Назначенных маршрутов пока нет. Все зарегистрированные магазины
            доступны на карте.
          </div>
        )}
      </div>
