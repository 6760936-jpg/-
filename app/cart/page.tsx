    ...
    <div className="container-page min-h-[70vh]">
      ...
      {items.length === 0 ? (
        <div className="surface-card border-dashed p-12 text-center">
          <h2 className="text-2xl font-semibold">Корзина пока пуста</h2>
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-4">
            {items.map(({ product, quantity }) => (
              <article
                key={product.id}
                className="surface-card grid gap-4 p-4 sm:grid-cols-[110px_1fr_auto] sm:items-center"
              >
                <ProductImage
                  src={product.image}
                  alt={product.name}
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-zinc-500">
                    {product.article} · минимум {product.minOrder} шт.
                  </p>
                  <button
                ...
                <div className="flex items-center justify-between gap-4 sm:block sm:text-right">
                  <div className="inline-flex items-center rounded-xl border border-zinc-300">
                    ...
                    <input
                      className="w-16 border-x border-zinc-300 py-2 text-center text-sm font-semibold outline-none"
                      type="number"
          ...
          <aside className="surface-card h-fit p-6">
            ...
            <Link href="/catalog" className="button-secondary mt-3 w-full">
              Продолжить покупки
            </Link>
          </aside>
        </div>
