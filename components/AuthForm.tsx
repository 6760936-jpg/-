  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (role && ["ADMIN", "DIRECTOR"].includes(role)) {
        destination = "/admin";
      } else if (role && ["FIELD", "DRIVER"].includes(role)) {
        destination = "/field";
    ...
    <form onSubmit={submit} className="surface-card space-y-5 p-6 sm:p-8">
      ...
      {isRegister && (
        <>
          ...
          <label className="block">
            <span className="field-label">Адрес магазина</span>
            <input
              className="input"
              name="address"
              required
          ...
          <label className="block">
            ...
            <input
              ...
              name="shelfCode"
              placeholder="Например, P-0007"
              maxLength={32}
            />
            <span className="mt-2 block text-xs text-zinc-500">
              Если наша полка уже установлена, укажите её номер. Если
              полки нет — оставьте поле пустым.
            </span>
          </label>
        </>
      )}
